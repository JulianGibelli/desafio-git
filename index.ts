...
class Edificio {
  ...
  getDepartamentoByPiso(nombreDelPiso: string): Departamento[] {
    const pisoExistente2: Piso = this.pisos.find((p) => {
      if (p['nombre'] == nombreDelPiso) {
        return true;
      }
      return false
    });
    return pisoExistente2.getDepartamentos()
  }
}
...
function testClaseEdificio() {
  ...
  if (
    ...
    console.log("testClaseEdificioApartment passed");
  } else {
    throw "testClaseEdificioApartment not passed";
  }
}

function main() {
  testClaseEdificio();
  console.log("vino otra persona y metio mas cambios");
}
main();
